...
# IMPORTANTE: Asegúrate de poner tu API KEY en Streamlit Secrets
try:
...
# =========================
# 2) DISEÑO CSS (Mantengo su estilo)
# =========================
st.markdown("""
    <style>
...
# =========================
ADN_LUIS = """
Eres Luis, papá chileno de Ignacia. Eres cariñoso, protector y muy orgulloso de ella.
Tu lenguaje debe incluir: 'bacán', 'se pasó', 'Pucha', 'Vivaldi', 'mi chiquitita', 'hijita linda'.
REGLAS:
1. Valida su emoción siempre.
2. Da respuestas cortas (máximo 3 frases).
3. No seas irónico ni pesado.
4. Si detectas peligro (daño, abuso), sé serio y dile que busque a un adulto de confianza.
"""

# =========================
# 4) LISTADOS (FOTOS Y CHISTES) - Mantengo sus listas exactas
# =========================
...
# =========================
# 5) MOTOR DE IA CORREGIDO
# =========================
def generar_respuesta_ia(estado, texto_usuario, historial):
    try:
        mensajes = [{"role": "system", "content": ADN_LUIS + f"\nEstado actual de Ignacia: {estado}"}]
        # Agregar historial compacto
        ...
        return "Pucha mi niñita, algo pasó con la conexión, pero sepa que la amo mucho. ¿Me cuenta por WhatsApp?"
...
if animo != "Seleccione":
    ...
    with st.spinner("Papi está escribiendo..."):
        mensaje = generar_respuesta_ia(animo, f"Me siento {animo}", st.session_state.chat_messages)
...
if user_text:
    ...
    with st.chat_message("assistant"):
        reply = generar_respuesta_ia(animo, user_text, st.session_state.chat_messages)
        st.write(reply)
