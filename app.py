...
# 2. DISEÑO CSS (Botón invisible ultra-prioritario)
st.markdown("""
<style>
    .stApp { background-color: white; }
    .portada-negra {
        position: fixed; top: 0; left: 0; width: 100vw; height: 100vh;
        background: black; display: flex; align-items: center; justify-content: center; z-index: 100;
    }
    .logo-sobre { position: absolute; width: 80%; max-width: 500px; z-index: 200; pointer-events: none; }
    
    /* EL BOTÓN INVISIBLE AHORA TIENE Z-INDEX 9999 (POR ENCIMA DE TODO) */
...
if not st.session_state.entrado:
    ...
    if "saludo" not in st.session_state: st.session_state.saludo = random.choice(APODOS)
    ...
    if st.button("🤡 ¡Papi, cuéntame un chiste!", key="joke"): st.info(random.choice(CHISTES))
    st.divider()
